...
;
     ...
     * Constructorbot
     */
    public function __construct($params, $imagePath)
    {
        ...
        $this->_set_params($params, $imagePath);
     ...
     */
    private function _set_params($params, $imagePath)
    {
        // echo $params; exit;
        
        // If enforcing params, ensure it's a match
        if ($this->config['enforce_presets'] and ! in_array($params, $this->config['presets']))
            throw new HttpException(404,'The requested URL '. App::instance()->http()->getURI() .' was not found on this server.');
